    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map((stat) => (
          <div
            key={stat.title}
            className={`${stat.bgColor} rounded-xl p-4 border ${stat.highlight ? 'border-emerald-200' : 'border-transparent'}`}
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600">{stat.title}</p>
                <p className={`text-2xl font-bold ${stat.highlight ? 'text-emerald-600' : 'text-gray-800'}`}>
                  {stat.value}
                </p>
              </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-amber-700">
            ...
            Configure seus ambientes, mesas e cardápio para começar a receber pedidos.
          </p>
        </div>
      </div>
    </div>
