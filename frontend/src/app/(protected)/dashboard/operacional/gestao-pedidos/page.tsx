...
const GestaoPedidosPage = () => {
  ...
  useEffect(() => {
    // Só carrega pedidos se não estiver no loading inicial e se ambientes já foram carregados
    if (!isLoading && ambientes.length > 0) {
      loadPedidos();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [ambienteSelecionado, isLoading]);
  ...
  const loadInitialData = async () => {
    try {
      ...
      // Carregar pedidos de todos os ambientes
      const todosAmbientes = await Promise.all(
        ambientesPreparo.map(amb => getPedidosPorAmbiente(amb.id))
      );
      const todosPedidos = todosAmbientes.flat();
      setPedidos(todosPedidos);
      
      logger.log(`✅ ${todosPedidos.length} pedidos carregados inicialmente`, {
        module: 'GestaoPedidosPage',
      });
    } catch (error) {
  ...
  const loadPedidos = async () => {
    try {
      ...
      // Se for 'todos', não passa ambienteId (busca tudo)
      // Se tiver UUID válido, passa o UUID
      let data: Pedido[] = [];
      
      if (ambienteSelecionado !== 'todos') {
        const resultado = await getPedidosPorAmbiente(ambienteSelecionado);
        data = Array.isArray(resultado) ? resultado : [];
      } else {
        // Buscar todos os ambientes e consolidar
        const todosAmbientes = await Promise.all(
          ambientes.map(amb => getPedidosPorAmbiente(amb.id))
        );
        data = todosAmbientes.flat();
      }
      
      setPedidos(data);
      logger.log(`✅ ${data.length} pedidos carregados`, {
        module: 'GestaoPedidosPage',
      });
    } catch (error) {
